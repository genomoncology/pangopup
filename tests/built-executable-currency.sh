...
if ! report=$(python3 "$scanner" builder "$repository" 2>"$fixture/builder.err"); then
    fail "the shared scanner refused a stale built executable: $(tr '\n' ' ' <"$fixture/builder.err")"
fi
read -r scanned guarded bad < <(
    python3 -c 'import json,sys; d=json.load(sys.stdin); print(d["files"], d["held"], d["bad"])' <<<"$report"
)
(( scanned > 0 && bad == 0 )) || fail 'the shared scanner returned an empty or inconsistent answer'

# Four harnesses reach into target/debug: executable delivery, inherited cache
# variables, model-cache limit inheritance, and production qualification.
harness_floor=4
