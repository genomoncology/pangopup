...
PORTABLE_QUALIFICATION := tests/built-executable-currency.sh tests/ci-platform-support.sh tests/ci-service-fixture-evidence.sh tests/ci-test-failure-evidence.sh tests/cli-spawn-cache-isolation.sh tests/inherited-cache-variables.sh tests/model-cache-layout-history.sh tests/model-cache-limit-inheritance.sh tests/negative-assertion-strength.sh tests/qualification-runner-cache-isolation.sh tests/recipe-spawn-cache-isolation.sh tests/release-help-contract.sh tests/route-disagreement-rate.sh tests/rust-literal-continuity.sh tests/shell-spawn-cache-isolation.sh tests/spec-block-execution.sh tests/spec-cargo-filter-evidence.sh tests/spec-download-cache-durability.sh tests/spec-refutation-evidence.sh tests/version-consistency-python39.sh tests/workflow-command-anchoring.sh tests/workflow-setting-anchoring.sh
