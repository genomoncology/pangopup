...
outside one fixed bounded FIFO model queue and exposes health/status. It reports one canonical active scoring identity through status and every result while preserving detailed route provenance and standalone CLI output. Status also reports the machine-readable scoring-request contract from the values that enforcement uses. Ticket
