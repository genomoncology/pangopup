...
Final review found that the second checker still allowed rounded inputs on the separate calculation, an appended precision contradiction, and swapped prose totals. Its `rounded` search also matched inside `unrounded`. The final checker examines every relevant precision statement, applies token-bound displayed and rounded checks to both calculations, and requires each exact result in its own calculation paragraph. Exact mutations cover those failures. A positive joint-locus statement that uses unrounded entropy and the existing paragraph reflow both pass.
