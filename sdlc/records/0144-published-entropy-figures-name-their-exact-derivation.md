...
The implementation did not run the analyzer, read the source corpus, or change an index decision. It read the retained Rust calculation and made the existing evidence explicit. A focused claim check reads the retained report and analyzer. Its mutations reject rounded-input arithmetic, three distinct alternate-record entropies, missing reference and joint terms, drift in either total, wrong display precision, and missing source attribution. A paragraph reflow passes.
