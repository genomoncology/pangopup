...
Design review corrected the original ticket before implementation. It named the public field and limited its semantics to the stable grouping and filter component. Independent code review mechanically verified every changed fixture and accepted the implementation without findings. The combined workspace test exposed two stale test-tool paths. The provenance guard now permits only the adjacent, value-matched `stable_gene` addition before applying the original byte hashes. The independent SNV fixture generator now writes the same field and still proves exact regeneration without using the production renderer. Independent re-review accepted both corrections.
