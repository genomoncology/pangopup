...
Hosted run 33863790902 proved the diagnostic and exposed the remaining defect. The macOS job passed. The Linux job passed lint, the ARM64 command build, and the portable service fixture, then reported that `local_install_is_atomic_reusable_and_lock_safe` could not reuse an installed asset when a compressed transport part had mode `000`. Transport inspection opened every part before deciding that the installed bundle could be reused. The corrected path validates part type, filesystem, and size through descriptor-relative no-follow metadata. Reuse does not open a part. Fresh installation still opens and authenticates every part before publication. Hosted CI run 33866509384 passed both macOS and Linux. Hosted container run 33866509387 passed Ubuntu 24.04 on AMD64 and ARM64.
