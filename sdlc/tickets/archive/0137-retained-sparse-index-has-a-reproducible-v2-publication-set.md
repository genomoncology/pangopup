...
- Each assembled bundle and reconstructed transport passes exhaustive certification. The isolated install reports SNV ready and runtime missing. Explicit and discovered lookup produce identical bytes for the 994 authoritative found or ambiguity requests. The six position-1 SNV misses return the checked explicit `not_found` results. Test each discovered miss separately: it exits 2, writes no stdout, and reports `MODEL_REJECTED` with `insufficient GRCh38 reference context` before runtime admission. A combined batch stops at its first rejection without partial output. Record this correction to the reviewed missing-runtime assumption without changing product behavior.
