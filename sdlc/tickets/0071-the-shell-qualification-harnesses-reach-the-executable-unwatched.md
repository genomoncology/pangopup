...
`tests/production-release-qualification.sh` is one of these harnesses, and it is the one run against a real service to qualify a release. That run happens on an operator's own machine.

The rule is that a harness naming the built executable holds a cache home of its own, not that a check reads each invocation's arguments to decide whether that invocation resolves a cache. Parsing argument shape out of shell is the fragile form and this ticket does not ask for it.
