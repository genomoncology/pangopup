...
- Each assembled bundle and reconstructed transport passes exhaustive certification. The isolated install reports SNV ready and runtime missing. Explicit and discovered command lookup agree on the 994 authoritative found or ambiguity regression requests. The six pure SNV misses return the checked explicit `not_found` results, while discovered lookup returns the expected missing-runtime error because this ticket installs no model-side profile.
