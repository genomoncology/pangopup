...
Boundary: do not change the measured constant, the `Retry-After` formula, admission accounting, queue capacity defaults, limits, HTTP statuses, response shapes, scoring, routing, or caching. Do not add a server-side timeout, a latency guarantee, a recommended client timeout, a per-request estimate, or configuration for either value.
