...
Conversion occurs before routing, cache lookup, admission, and inference. Both the exact-edit and existing anchored forms converge on one `Grch38Variant`. Submitting equivalent forms produces the same score, provenance, response allele, queue weight, and persistent-cache identity. The core layer owns the typed exact-edit values and errors. The engine owns conversion against a supplied reference provider. CLI and HTTP parse and render. The cache remains unchanged.
...
- `README.md`, CLI help, `spec/cli.md`, `spec/model-routing.md`, the HTTP contract and `spec/http-service.md` document the grammar and worked examples. `architecture/decisions/0003-standalone-genomic-variant-boundary.md` records the public boundary. `architecture/runtime-data.md` records conversion before cache and inference.
