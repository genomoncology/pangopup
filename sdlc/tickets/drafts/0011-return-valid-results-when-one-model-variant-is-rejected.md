...
- The same response carries a stable typed rejection for the rejected input in its original position.
...
Boundary: do not add GRCh37 input, allele normalization, retry behavior, asynchronous jobs, or partial results after an operational failure. Do not change the CLI, score calculation, lookup-first routing, result order, caching identity, or the behavior of a batch whose variants all produce normal results.
