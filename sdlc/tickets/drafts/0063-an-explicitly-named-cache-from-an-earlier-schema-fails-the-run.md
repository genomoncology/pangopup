...
Closed inside ticket 0058 by its code review. `USER_VERSION` now stamps the file layout, and a file carrying a layout an earlier release wrote is discarded whole and reported, the way any other setup change is, instead of failing the run. A foreign, damaged or later-layout database is still refused rather than deleted. Measured: a `--model-cache` path holding a v0.4.x cache now exits 0, prints one score line, names the discarded file on standard error, and refills. Nothing below is still open. The proof is authored: `a_default_cache_an_earlier_release_wrote_is_discarded_and_reported`, `a_chosen_cache_an_earlier_release_wrote_is_discarded_rather_than_fatal` and `a_foreign_damaged_or_later_cache_is_refused_and_never_deleted` in `crates/pangopup-cli/tests/model_cache_setup.rs`. What keeps the earlier-layout list honest at the next bump is draft 0066.
