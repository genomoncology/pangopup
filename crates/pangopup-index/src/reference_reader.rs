//! The single `PGRREF01` mmap reader and provider.

use crate::reference_wire::{
    ACGT_ASCII, AmbiguityRun, CONTIG_COUNT, DENSE_OFFSET, DIRECTORY_BYTES, DIRECTORY_ENTRY_BYTES,
    DirectoryEntry, ENCODING, HEADER_BYTES, MAGIC, MAX_AMBIGUITY_RUNS, MAX_MANIFEST_BYTES,
    MAX_MEMBER_BYTES, MAX_NOTICE_BYTES, MINI_NOTICE, MINI_PROFILE, PRODUCTION_NOTICE,
    PRODUCTION_PROFILE, REFERENCE_FORMAT, REFERENCE_SCHEMA, ROUTE_TEST_NOTICE, ROUTE_TEST_PROFILE,
    RUN_BYTES, ReferenceIndexError, ReferenceManifest, VERSION, align8,
    canonical_reference_manifest_bytes, iupac_ascii, le_u16, le_u32, le_u64,
    production_contig_lengths, production_fasta_url, production_report_url, reference_bundle_id,
    required_accession, valid_sha,
};
use memmap2::Mmap;
use pangopup_core::{
...
/// Map an already-authenticated installed member without reopening any path.
///
/// # Safety
///
/// The caller must have authenticated `manifest_bytes`, `notice_bytes`, and
/// `reference` as one canonical, bounded, immutable installed bundle. In
/// particular, the descriptor must be the exact regular single-link member
/// whose size and digest are declared by the manifest.
pub(crate) unsafe fn open_held_installed(
    manifest_bytes: &[u8],
    notice_bytes: &[u8],
    reference: File,
) -> Result<ReferenceBundleOpen, ReferenceIndexError> {
    ReferenceBundleOpen::open_authenticated(manifest_bytes, notice_bytes, reference)
}
...
impl ReferenceBundleOpen {
    ...
    fn open_identified_with(
        ...
        let provider = Self::open_authenticated(&manifest_bytes, &notice, reference.try_clone()?)?;
    ...
    pub fn open_qualification(
        ...
        Self::open_authenticated(manifest_bytes, notice_bytes, reference.try_clone()?)
    }
    ...
    pub fn open_qualification_audited(
        ...
        Self::open_authenticated_audited(manifest_bytes, notice_bytes, reference.try_clone()?)
    }
...
mod tests {
    ...
    use std::{io::Write, os::unix::fs::symlink, path::PathBuf};
