//! Streaming validation and characterization of published Pangolin SNV score files.
...
impl TotalSummary {
    pub(crate) fn add(&mut self, file: FileSummary) {
        self.genes += 1;
        self.rows += file.rows;
        self.loci += file.loci;
...
pub(crate) fn index_locus(locus: SourceLocus) -> InputLocus {
    match locus {
...
pub(crate) fn discover_members(source_dir: &Path) -> Result<Vec<(OsString, PathBuf)>, SourceError> {
...
pub(crate) fn parse_member_gene(filename: &str) -> Result<EnsemblGeneId, SourceError> {
