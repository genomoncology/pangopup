...
fn parse_contig(value: &str) -> Option<Grch38Contig> {
    if let Ok(contig) = value.parse::<Grch38Contig>() {
        return Some(contig);
    }
    (1_u8..=25).find_map(|code| {
        let contig = Grch38Contig::from_code(code).ok()?;
        contig_spellings(contig).accepts(value).then_some(contig)
    })
}

#[derive(Clone, Copy)]
struct ContigSpellings {
...
fn contig_spellings(contig: Grch38Contig) -> ContigSpellings {
    const BARE: [&str; 25] = [
