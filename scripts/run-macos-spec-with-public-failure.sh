...
log="$RUNNER_TEMP/make-spec.log"
set +e
make spec 2>&1 | tee "$log"
statuses=("${PIPESTATUS[@]}")
set -e
...
if [[ -f "$log" ]] && summary=$(tail -n 120 "$log" | tail -c 1300); then
    ...
    summary='macOS spec log is unavailable'
fi
summary=${summary//'%'/'%25'}
summary=${summary//$'\r'/'%0D'}
summary=${summary//$'\n'/'%0A'}
printf '::error file=Makefile,line=115,title=macOS make spec failure::%s\n' "$summary" || true
