...
The HTTP service derives `pangopup.active-scoring-identity.v1` from this runtime-profile identity, the running software version, and the effective service CPU policy. The full SHA-256 stays stable across machines and restarts with the same three inputs. It changes when any admitted asset, answer-affecting profile fact, software release, or effective thread policy changes. Operational details such as workers, queue state, cache, paths, listener, process, and host do not enter it. `data_set_version` gives a consumer one concise version value that no deployment setting moves. The service derives it from this runtime-profile identity and the running software version, and publishes both inputs on the status route. Detailed route provenance continues to identify how an individual result was obtained.
