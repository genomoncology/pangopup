...
Health and status routes are `/livez`, `/readyz`, and `/v1/status`. Its `request_contract` is the machine-readable source for client limits and input forms. Status and every score item share the `scoring_identity`. Store it as the data-set version when a system has one version field. Retain detailed provenance for route and component audits. `/v1/score` requires one `Content-Type: application/json` field; case and legal parameters are accepted. Other, malformed, missing, and repeated content types receive HTTP 415. The route accepts 1–100 variants. It accepts at most the reported `max_uncached_model_items`, which equals the smaller of 10 uncached model variants and queue capacity. Larger requests receive HTTP 422 with `MODEL_BATCH_TOO_LARGE`. A successful batch returns one ordered outcome per input and HTTP 200 when at least one item is normal. A rejected item has `status: "rejected"`, empty `records` and `source_reference_ambiguities`, no provenance, and `error.code: "MODEL_REJECTED"`. An all-rejected request returns HTTP 422. Scoring, cache, worker, and readiness failures invalidate the complete request. Temporary queue saturation returns HTTP 429 with `Retry-After`. The service has no built-in authentication or TLS. Keep it on loopback or behind an authenticated TLS reverse proxy with suitable limits. Use an external process manager for supervised lifecycle behavior.
