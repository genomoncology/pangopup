...
Health and status routes are `/livez`, `/readyz`, and `/v1/status`. Its `request_contract` is the machine-readable source for client limits and input forms. Status and every returned score item carry the same `scoring_identity`. Store it as the data-set version when a system has one version field. Retain detailed provenance for route and component audits. `/v1/score` requires one `Content-Type: application/json` field; case and legal parameters are accepted. Other, malformed, missing, and repeated content types receive HTTP 415. The route accepts 1–100 variants and at most 10 uncached model variants. A successful batch returns one ordered outcome per input and HTTP 200 when at least one item is normal. A rejected item has `status: "rejected"`, empty `records` and `source_reference_ambiguities`, no provenance, and `error.code: "MODEL_REJECTED"`. An all-rejected request returns HTTP 422. Scoring, cache, worker, and readiness failures invalidate the complete request. Transient queue saturation returns HTTP 429 with one decimal-seconds `Retry-After`; an overweight request omits it. The service has no built-in authentication or TLS. Keep it on loopback or behind an authenticated TLS reverse proxy with suitable limits. Use an external process manager for supervised lifecycle behavior.
