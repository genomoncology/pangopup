...
The package-only attempt targeted commit
`342fa6e6a786c89ca03e98cfcb4935e4e4e7ca46`; exact remote `ci` run
`30653571561` passed. Package run `30653836700` built the release executables,
then stopped in deterministic SBOM generation because offline Cargo metadata
...
- SBOM dependency boundary: one `cargo fetch --locked` followed by both
  CycloneDX passes with Cargo network access disabled
