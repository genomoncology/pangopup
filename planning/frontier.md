...
Ticket 0142 makes cache-isolation checks compare complete environment operands and normalized lexical paths. The checks reject unsupported shell evaluation before expansion, preserve the relative `ORT_CACHE_DIR` exception, and classify dot segments, parent segments, equal paths, sibling prefixes, and trailing separators consistently on macOS and Linux. Drafts 0107 and 0108 are archived.

Ticket 0149 closes that command-list gap. The download-cache check rejects unquoted, unescaped shell separators on cache-deciding recipe lines with a Makefile path and line number. Quoted and escaped separators in ordinary arguments remain accepted. The focused fixture covers semicolons, conditionals, background separators, and pipelines.
...
Ticket 0145 binds the route-disagreement substitution coverage to the committed manifest on every lint run. The existing checker parses submitted SNVs and holds 2,615 transversions, zero transitions, and the four directed counts against the retained artifact and both public documents. A matched manifest and raw-record transition mutation fails on the class count. Malformed, equal-base, and non-SNV rows fail with a substitution-class diagnostic. Draft 0105 is archived.
...
all-overlap requests. Exhaustive certification dispatches to either admitted
format and reconstructs the same logical stream and counts. Fixed-only
measurement accessors reject sparse input as incompatible. The CLI opens once,
